...
The `ConnectivityTask` polls `GTV_HasInternet` on a timer (every `NET_RETRY_MS` = 30 000 ms) and updates `m.global.isOnline`. The flow is:

1. `ConnectivityTask` calls `GTV_HasInternet(m.global.activeServer)`.
2. If `false`, it sets `m.global.isOnline = false` and shows the offline dialog.
3. When `true` again, it restores `m.global.isOnline = true` and triggers a reconnect.
